...
const SelectMenuForm = ({
  ...
  const selectSubMenu = (item) => {
    ...
    const subMenuType = subMenuMap[item] || "";  
    setValue("subMenuType", subMenuType);  
    setMenuType((prev) => {
      // Handle "All" case separately
      if (item === "All") {
        // Clear the Sub_Menu_Type and fetch all items
        getMenuList({ ...prev, Sub_Menu_Type: "" });
        return { ...prev, Sub_Menu_Type: "" };
      }  
      // Handle other submenus (Veg, NonVeg, Drink)
      if (prev.Sub_Menu_Type !== subMenuType) {
        getMenuList({ ...prev, Sub_Menu_Type: subMenuType });
      }
      return { ...prev, Sub_Menu_Type: subMenuType };
    });
    console.log("selectSubMenu_", item);    
  };
    ...
    >
      ...
      <div className="flex gap-4 justify-center mb-4 menu-select-icons">
        <Box
          ...
          className={`menu-select-icon ${menu ? "active" : ""}`}
        >
